...
for ($i = 1; $i -le 64; $i++) {
    ...
    erl +S $i -noshell -s benchmark test_sharded_cached_readwrite -s init stop | Out-File -Encoding utf8 "$DirName/sharded-cached-rw$i.txt"

    Write-Output "---"
    Write-Output "> readwrite worker, sharded, cached, $i threads"
    erl +S $i -noshell -s benchmark test_worker_sharded_cached_readwrite -s init stop | Out-File -Encoding utf8 "$DirName/worker-sharded-cached-rw$i.txt"
    ...
    erl +S $i -noshell -s benchmark test_sharded_cached_readonly -s init stop | Out-File -Encoding utf8 "$DirName/sharded-cached-r$i.txt"

    Write-Output "---"
    Write-Output "> readonly worker, sharded, cached, $i threads"
    erl +S $i -noshell -s benchmark test_worker_sharded_cached_readonly -s init stop | Out-File -Encoding utf8 "$DirName/worker-sharded-cached-r$i.txt"
    ...
    erl +S $i -noshell -s benchmark test_sharded_cached_mixedload -s init stop | Out-File -Encoding utf8 "$DirName/sharded-cached-mix$i.txt"

    Write-Output "---"
    Write-Output "> mixed load, worker, sharded, cached, $i threads"
    erl +S $i -noshell -s benchmark test_worker_sharded_cached_mixedload -s init stop | Out-File -Encoding utf8 "$DirName/worker-sharded-cached-mix$i.txt"
}
